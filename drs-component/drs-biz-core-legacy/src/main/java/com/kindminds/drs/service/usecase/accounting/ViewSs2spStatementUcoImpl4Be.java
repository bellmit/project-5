...
import com.kindminds.drs.v1.model.impl.report.ProfitShareSubtractionAdvertisingCostReportImpl;
import com.kindminds.drs.v1.model.impl.report.ProfitShareSubtractionMarketingActivityExpenseReportImpl;
import com.kindminds.drs.v1.model.impl.report.ProfitShareSubtractionOtherRefundReportImpl;
...
@Service("ViewSs2spStatementUcoImpl4Be")
public class ViewSs2spStatementUcoImpl4Be implements ViewSs2spStatementUco, ViewSs2spStatementInternalUco {
    ...
    private Ss2spStatementReportFactory selectFactory(BillStatementType type, String statementName){
        ...
        String factoryBeanId = Ss2spStatementReportFactory.ReportVersion.fromVersionNumber(versionNumber).getFactoryBeanId();
    ...
    public Map<String,String> getSupplierKcodeToNameMap() {
        if(Context.getCurrentUser().isDrsUser()) return this.CompanyDao.querySupplierKcodeToShortEnUsNameMap();
        return null;
    }

    @Override
    public Map<String,String> getAllCompanyKcodeToLocalNameMap() {
        return CompanyDao.queryAllCompanyKcodeToLocalNameMap();
    }
    ...
    @Override
    public Ss2spProfitShareReport getSs2spProfitShareReportInternal(BillStatementType type, String statementName) {
        return this.selectFactory(type,statementName).createSs2spProfitShareReport(type,statementName);
    }
    ...
    public Ss2spSkuProfitShareDetailReport getSs2spSkuProfitShareDetailReport(BillStatementType type, String statementName, String countryCode, String sku) {
       ...
       List<Object []> columnsList =  this.dao.querySs2spSkuProfitShareDetailReportShippedItem(start, end, country, sku);
        ...
        report.setShippedItems(shippedItems);
        List<Object []> columnsList1 = this.dao.querySs2spSkuProfitShareDetailReportRefundedItem(start, end, country, sku);
        ...
        report.setRefundedItems(refundedItems);
        return report;
    }
    ...
    @Override
    public ProfitShareSubtractionAdvertisingCostReport getSs2spAdvertisingCostReport(String statementName, String countryCode) {

        List<Object []> resultColumnArrayList =
                dao.queryAdvertisingCostReportItems(statementName, countryCode);

        List<ProfitShareSubtractionAdvertisingCostReport.ProfitShareSubtractionAdvertisingCostReportLineItem> items = new ArrayList<>();
        for(Object[] columns : resultColumnArrayList) {
            ...
            BigDecimal vatRate = (BigDecimal) columns[3];
            BigDecimal vatAmount = (BigDecimal) columns[4];
            BigDecimal subTotal = amount.add(vatAmount);

            items.add(new ProfitShareSubtractionAdvertisingCostReportLineItemImpl(
                    itemNote, currencyName, amount, vatRate, subTotal));
        }

        ProfitShareSubtractionAdvertisingCostReportImpl profitShareSubtractionAdvertisingCostReport = new ProfitShareSubtractionAdvertisingCostReportImpl();
        BigDecimal total = new BigDecimal(0);
        for(ProfitShareSubtractionAdvertisingCostReport.ProfitShareSubtractionAdvertisingCostReportLineItem item : items) {
            total = total.add(item.getSubTotal());
        }
        total = total.setScale(6);
        StatementInfo statementInfo = getInfo(BillStatementType.OFFICIAL, statementName);
        profitShareSubtractionAdvertisingCostReport.setDateStart(statementInfo.getDateStart());
        profitShareSubtractionAdvertisingCostReport.setDateEnd(statementInfo.getDateEnd());
        profitShareSubtractionAdvertisingCostReport.setIsurKcode(statementInfo.getIsurKcode());
        profitShareSubtractionAdvertisingCostReport.setRcvrKcode(statementInfo.getRcvrKcode());
        profitShareSubtractionAdvertisingCostReport.setTotal(total);
        profitShareSubtractionAdvertisingCostReport.setLineItems(items);

        return profitShareSubtractionAdvertisingCostReport;
    }

    @Override
    public ProfitShareSubtractionOtherRefundReport getSs2spOtherRefundReport(String statementName, String countryCode) {
        List<Object []> resultColumnArrayList = dao.queryOtherRefundReportItems(statementName, countryCode);

        List<ProfitShareSubtractionOtherRefundReport.ProfitShareSubtractionOtherRefundReportLineItem> items = new ArrayList<>();
        for(Object[] columns : resultColumnArrayList) {
            String itemNote = (String) columns[0];
            String currencyName = (String) columns[1];
            BigDecimal subtotal = (BigDecimal) columns[2];
            items.add(new ProfitShareSubtractionOtherRefundReportLineItemImpl(itemNote, currencyName, subtotal));
        }

        ProfitShareSubtractionOtherRefundReportImpl profitShareSubtractionOtherRefundReport = new ProfitShareSubtractionOtherRefundReportImpl();
        BigDecimal total = new BigDecimal(0);
        for(ProfitShareSubtractionOtherRefundReport.ProfitShareSubtractionOtherRefundReportLineItem item : items) {
            total = total.add(item.getAmount());
        }
        total = total.setScale(6);
        StatementInfo statementInfo = getInfo(BillStatementType.OFFICIAL, statementName);
        profitShareSubtractionOtherRefundReport.setDateStart(statementInfo.getDateStart());
        profitShareSubtractionOtherRefundReport.setDateEnd(statementInfo.getDateEnd());
        profitShareSubtractionOtherRefundReport.setIsurKcode(statementInfo.getIsurKcode());
        profitShareSubtractionOtherRefundReport.setRcvrKcode(statementInfo.getRcvrKcode());
        profitShareSubtractionOtherRefundReport.setTotal(total);
        profitShareSubtractionOtherRefundReport.setLineItems(items);

        return profitShareSubtractionOtherRefundReport;
    }

    @Override
    public ProfitShareSubtractionMarketingActivityExpenseReport getSs2spMarketingActivityExpenseReport(String statementName, String countryCode) {

        List<Object []> resultColumnArrayList =
                dao.queryMarketingActivityExpenseReportItems(statementName, countryCode);

        List<ProfitShareSubtractionMarketingActivityExpenseReport.ProfitShareSubtractionMarketingActivityExpenseReportLineItem>
                items = new ArrayList<>();
        for(Object[] columns : resultColumnArrayList) {
            String itemNote = (String) columns[0];
            String currencyName = (String) columns[1];
            BigDecimal subtotal = (BigDecimal) columns[2];
            items.add(new ProfitShareSubtractionMarketingActivityExpenseReportLineItemImpl(itemNote, currencyName, subtotal));
        }

        ProfitShareSubtractionMarketingActivityExpenseReportImpl profitShareSubtractionMarketingActivityExpenseReport = new ProfitShareSubtractionMarketingActivityExpenseReportImpl();
        BigDecimal total = new BigDecimal(0);
        for(ProfitShareSubtractionMarketingActivityExpenseReport.ProfitShareSubtractionMarketingActivityExpenseReportLineItem item : items) {
            total = total.add(item.getAmount());
        }
        total = total.setScale(6);
        StatementInfo statementInfo = getInfo(BillStatementType.OFFICIAL, statementName);
        profitShareSubtractionMarketingActivityExpenseReport.setDateStart(statementInfo.getDateStart());
        profitShareSubtractionMarketingActivityExpenseReport.setDateEnd(statementInfo.getDateEnd());
        profitShareSubtractionMarketingActivityExpenseReport.setIsurKcode(statementInfo.getIsurKcode());
        profitShareSubtractionMarketingActivityExpenseReport.setRcvrKcode(statementInfo.getRcvrKcode());
        profitShareSubtractionMarketingActivityExpenseReport.setTotal(total);
        profitShareSubtractionMarketingActivityExpenseReport.setLineItems(items);
