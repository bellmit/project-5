...
import com.kindminds.drs.data.pipelines.api.message.{ImportRemovalOrderRpt, RegisterCommandHandler}
...
object AmzRemovalOrderHandler {

  def props(drsCmdBus:ActorRef): Props =
    Props(new AmzRemovalOrderHandler(drsCmdBus))
}

class AmzRemovalOrderHandler(drsCmdBus:ActorRef) extends Actor with ActorLogging {
  val name = self.path.name

  drsCmdBus ! RegisterCommandHandler(name,classOf[ImportRemovalOrderRpt].getName ,self)
  ...
  override def receive = {
    case ImportRemovalOrderRpt(path) =>
      process(path)
  ...
  val filePath = "C:/aie/removal_order_detail_UK20211125.txt"
...
//    val byteStringSource =  Source.single(bs)
    ...
    r onComplete {
      case Success(x) => {
        ...
        val gr =x.groupBy(x => (x.get("order-id").get ,
          x.get("sku").get))
        ...
        gr.foreach(x =>{
          ...
          val existId = queryExistId(x._1._1 , x._1._2)
...
//          println("time : "+LocalDate.parse(x._1._1,dateFormatter))
    ...
    def queryExistId(oi:String,sku:String):Array[Int]={
      ...
      val query =   sql"""
                select id
                FROM inventory.amazon_removal_order
                where order_id='#${oi}'
                and sku ='#${sku}'
                order by id asc
                  """.as[String]
    ...
    def insert(y:Map[String,String] )={
      val master =   sqlu"""
                     INSERT INTO inventory.amazon_removal_order(
	   report_marketplace, country, request_date, order_id, order_type, order_status, last_updated_date, sku, fnsku, disposition,
	   requested_quantity, cancelled_quantity, disposed_quantity, shipped_quantity, in_process_qunatity, removal_fee, currency)
	    VALUES ('#${reportMarketplace}','#${getCountry(y.get("order-id").get)}', '#${y.get("request-date").get}', '#${y.get("order-id").get}', '#${y.get("order-type").get}',
     '#${y.get("order-status").get}', '#${y.get("last-updated-date").get}', '#${y.get("sku").get}', '#${y.get("fnsku").get}','#${y.get("disposition").get}',
     '#${getNullQuantity(y.get("requested-quantity").get)}','#${getNullQuantity(y.get("cancelled-quantity").get)}',
     '#${getNullQuantity(y.get("disposed-quantity").get)}','#${getNullQuantity(y.get("shipped-quantity").get)}',
     '#${y.get("in-process-quantity").get}', '#${getNullQuantity(y.get("removal-fee").get)}', '#${y.get("currency").get}'); """
    ...
    def update(y:Map[String,String],id:Int )={
      val master =   sqlu"""
             update inventory.amazon_removal_order
	   SET report_marketplace='#${reportMarketplace}', country ='#${getCountry(y.get("order-id").get)}', request_date='#${y.get("request-date").get}',
	   order_type='#${y.get("order-type").get}', order_status='#${y.get("order-status").get}', last_updated_date='#${y.get("last-updated-date").get}',
	   fnsku='#${y.get("fnsku").get}', disposition='#${y.get("disposition").get}',
	   requested_quantity='#${getNullQuantity(y.get("requested-quantity").get)}', cancelled_quantity='#${getNullQuantity(y.get("cancelled-quantity").get)}',
	   disposed_quantity='#${getNullQuantity(y.get("disposed-quantity").get)}', shipped_quantity='#${getNullQuantity(y.get("shipped-quantity").get)}',
	   in_process_qunatity='#${getNullQuantity(y.get("in-process-quantity").get)}', removal_fee='#${getNullQuantity(y.get("removal-fee").get)}',
	   currency='#${y.get("currency").get}'
	    where id ='#${id}' ;"""
    ...
    def delete(id:Int)={
      val del =sql"""
          delete from inventory.amazon_removal_order where id='#${id}'
           """.as[String]
    ...
    def getNullQuantity(qty:String): String={
      if (qty==""){
        "0"
      }else{
        qty
