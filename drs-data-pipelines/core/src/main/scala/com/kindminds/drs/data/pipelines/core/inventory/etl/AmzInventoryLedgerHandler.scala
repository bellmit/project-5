...
import com.kindminds.drs.data.pipelines.api.message.{ImportInventoryLedgerRpt, RegisterCommandHandler}
...
object AmzInventoryLedgerHandler {

  def props(drsCmdBus:ActorRef): Props =
    Props(new AmzInventoryLedgerHandler(drsCmdBus))
}


class AmzInventoryLedgerHandler(drsCmdBus:ActorRef) extends Actor with ActorLogging {
  val name = self.path.name

  drsCmdBus ! RegisterCommandHandler(name,classOf[ImportInventoryLedgerRpt].getName ,self)
  ...
  override def receive = {
    case ImportInventoryLedgerRpt(path) =>
      process(path)
  ...
  val filePath = "C:/aie/US_InventoryLedger20210101-20211123.txt"
...
//    val byteStringSource =  Source.single(bs)
    ...
    r onComplete {
      case Success(x) => {
        ...
        val gr =x.groupBy(x => (x.get("Date").get ,x.get("MSKU").get,
          x.get("Event Type").get,x.get("Reference ID").get,x.get("Country").get))


        gr.foreach(x =>{

          val grCount=x._2.size
          val existId = queryExistId(x._1._1 , x._1._2 , x._1._3 , x._1._4 , x._1._5)
...
//          println("time : "+LocalDate.parse(x._1._1,dateFormatter))
    ...
    def queryExistId(date:String,msku:String,et:String,ri:String,country:String):Array[Int]={
      ...
      val query =   sql"""
                select id
                FROM inventory.amazon_inventory_ledger
                where date='#${LocalDate.parse(date,dateFormatter)}'
                and msku='#${msku}'
                and event_type='#${et}'
                and reference_id='#${ri}'
                and country='#${country}'
    ...
    def insert(y:Map[String,String] )={
      val master =   sqlu"""
                   INSERT INTO inventory.amazon_inventory_ledger( date,
	  fnsku, asin, msku, title, event_type, reference_id, quantity, fulfillment_center, disposition, reason, country)
    VALUES('#${LocalDate.parse((y.get("Date").get),dateFormatter)}', '#${y.get("FNSKU").get}', '#${y.get("ASIN").get}', '#${y.get("MSKU").get}', '#${y.get("Title").get.replace("'","''")}',
      '#${y.get("Event Type").get}', '#${y.get("Reference ID").get}', '#${y.get("Quantity").get}',
      '#${y.get("Fulfillment Center").get}', '#${y.get("Disposition").get}', '#${y.get("Reason").get}',
      '#${y.get("Country").get}'); """
    ...
    def update(y:Map[String,String],id:Int )={
      val master =   sqlu"""
             update inventory.amazon_inventory_ledger
	    SET fnsku='#${y.get("FNSKU").get}', asin='#${y.get("ASIN").get}', title='#${y.get("Title").get.replace("'","''")}',
      quantity='#${y.get("Quantity").get}' ,fulfillment_center='#${y.get("Fulfillment Center").get}' ,
	    disposition='#${y.get("Disposition").get}' , reason='#${y.get("Reason").get}'
	    where id ='#${id}' ;"""
    ...
    def delete(id:Int)={
      val del =sql"""
          delete from inventory.amazon_inventory_ledger where id='#${id}'
           """.as[String]
