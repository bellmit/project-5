...
import com.kindminds.drs.v1.model.impl.ManageFbaInventoryReportItemImpl;
...
@Service
public class ManageFbaInventoryFileImporterImpl implements ManageFbaInventoryFileImporter {

    @Autowired
    private ManageFbaInventoryFileImporterDao dao;
...
//			}
        ...
        String region  = countryCode.equals("US") || countryCode.equals("CA") ? "NA" : "EU";
        ...
        for(CSVRecord record : getRecordsFromFile(fileData)){
            ...
            if (dao.queryDataExists(snapshotDate, countryCode, fnsku)) {
                continue;
            }

            String marketplaceSku = record.get(headerNames.get("sku"));
            String codeByDrs = dao.queryDrsSkuByMarketplaceSku(marketplaceSku, marketplaceId);
            if (codeByDrs == null) {
                continue;
            }
