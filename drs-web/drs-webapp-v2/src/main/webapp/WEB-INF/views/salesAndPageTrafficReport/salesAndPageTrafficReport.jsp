...
<div class="max-width">
	<div class="container-fluid">
		...
		<div class="row">
			<div class="col-md-12">
			<form:form action="${pageContext.request.contextPath}/SalesAndPageTrafficReport" method="GET" name="SalesAndPageTrafficReport"
				id="salesAndPageTrafficReportForm" modelAttribute="SalesAndPageTrafficReport">							    
					...
					<div class="form-group">
						<label for="supplierKcode"><spring:message code="salesAndPageTrafficReport.supplier" /></label>
						...
						<form:select id="supplierKcode" class="form-control" path="supplierKcode">
							...
							<form:option value="${supplierKcodeName.key}">${supplierKcodeName.key} ${supplierKcodeName.value}</form:option>
							</c:forEach>
						</form:select>
					</div>
					</sec:authorize>
					...
					<div class="form-group">
					<label for="baseProduct"><spring:message code="salesAndPageTrafficReport.BPandSKU" /></label>
					...
					<div class="form-group">
					<label for="sku"><spring:message code="salesAndPageTrafficReport.SKU" /></label>
					...
					<div class="form-group">
					<label for="marketplaceId"><spring:message code="salesAndPageTrafficReport.marketplace" /></label>
					...
					<div class="form-group">
					<label for="timeScale"><spring:message code="salesAndPageTrafficReport.timeScale" /></label>
					<form:select class="form-control" path="timeScale">
							<c:forEach var="timeScale" items="${timeScales}">
							<form:option value="${timeScale.key}"><spring:message code="salesAndPageTrafficReport.${timeScale}" /></form:option>
							</c:forEach>
						</form:select>		
					</div>			

					<div class="form-group">
					<label for="trafficType"><spring:message code="salesAndPageTrafficReport.trafficType" /></label>
					<form:select id="trafficType" class="form-control" path="trafficType">
							<c:forEach var="trafficType" items="${trafficTypes}">
							<form:option value="${trafficType.key}"><spring:message code="salesAndPageTrafficReport.${trafficType}" /></form:option>
							</c:forEach>																	
						</form:select>
					</div>	
						
					<div class="form-group">
					<label for="dateRange"><spring:message code="salesAndPageTrafficReport.timeRange" /></label>
					<form:select id="dateRange" class="form-control selectDateRange" path="dateRange">
								<c:forEach var="dateRange" items="${dateRanges}">
								<form:option class="dateRangeList" value="${dateRange.key}"><spring:message code="salesAndPageTrafficReport.${dateRange}" /></form:option>
								</c:forEach>	
							</form:select>
					</div>	
					...
					<form:radiobutton class="totalOrAverage" path="totalOrAverage" value="1" />
							<label><spring:message code="salesAndPageTrafficReport.TOTAL" /></label>
							<span id="radioTotalAvg">
							<form:radiobutton class="totalOrAverage" path="totalOrAverage" value="2" />
							<label><spring:message code="salesAndPageTrafficReport.AVERAGE" /></label>
							</span>
					</div>

							<div class="showOnCustomize form-group">
							<label><spring:message code="campaignPerformance.startDateInput" /></label>
							...
							<div class="showOnCustomize form-group">
							<label><spring:message code="campaignPerformance.endDateInput" /></label>
			...
			<input type="hidden" id="selectedSkus" name="selectedSkus" value="${selectedSkus}" />
			<input type="hidden" id="selectedProducts" name="selectedProducts" value="${selectedProducts}" />
			<button class="btn btn-primary" style="float:right" type="submit"><i class="fas fa-search"></i> <spring:message code="salesAndPageTrafficReport.search" /></button>
			<input id="orderExport" class="btn btn-default mr-2" style="float:right"  type="submit" value="<spring:message code='customerOrder.export'/>" />
			</div>
		...
		<div class="col-md-6">
			<c:choose>
				<c:when test="${conditions.totalOrAverage eq 1}">
					<h4><b><spring:message code="salesAndPageTrafficReport.TOTAL" /></b></h4>
				</c:when>
				<c:when test="${conditions.totalOrAverage eq 2}">
					<h4><b><spring:message code="salesAndPageTrafficReport.AVERAGE" /></b></h4>
				</c:when>
			</c:choose>
		</div>
			...
			<table id="salesAndPageTrafficReport" class="table table-withoutBorder">
				<thead>
					<tr>
						...
						<th class="text-right"><spring:message code="salesAndPageTrafficReport.orderedProductSales" /> (${report.currency})</th>
						<th class="text-right"><spring:message code="salesAndPageTrafficReport.unitsOrdered" /></th>						
					</tr>	
				</thead>
				<tbody>
