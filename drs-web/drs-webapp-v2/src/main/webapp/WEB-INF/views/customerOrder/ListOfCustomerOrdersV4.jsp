...
<head>
  ...
  <script>
    ...
    function resetByDRSStaff() {
      $("#supplier,#amazonMerchantSKU,#asin,#salesChannel,#orderStatus,#orderDateStart,#orderDateEnd,#transactionDateStart,#transactionDateEnd,#marketplaceOrderId,#customerName,#buyerEmail,#promotionId").val("");
      $('#baseProduct').empty();
      $('#SKU').empty();
    ...
    function resetBySupplier() {
      $("#baseProduct,#asin,#salesChannel,#orderStatus,#orderDateStart,#orderDateEnd,#transactionDateStart,#transactionDateEnd,#marketplaceOrderId,#customerName,#promotionId").val("");
      $('#SKU').empty();
      $('<option>').val("").text("All").appendTo('#SKU');
    };
    ...
    jQuery(window).on("load", function (e) {
      ...
      /* Enable Select2 selection box */
      // Select2 does not support multiple ids at the same time, it seems
      $("#supplier").select2({
        theme: "bootstrap"
      });
    });
    ...
    $(function () {
      /* New datepicker */
      $("#orderDateStart, #orderDateEnd, #transactionDateStart, #transactionDateEnd").datetimepicker(
        {
        ...
        content: function () {
          ...
          if ($(this).attr(
            ...
            $.ajax({
              ...
              dataType: "json",
              async: false,
              success: function (amazonOrderItemDetail) {
                src
                  .find(
  ...
  <title>
    <spring:message code="customerOrder.title" /> - DRS</title>
</head>
  ...
  <div class="container-fluid">
    ...
    <div class="card">
      <div class="card-body">
        ...
        <!-- Customer order form -->
        <form:form method="GET" name="CustomerOrder" class="form-horizontal text-left" modelAttribute="CustomerOrder">
          ...
          <div class="row my-3">
            <div class="col-md-6">
              <div class="form-group elasticSearch">
                <label for="searchTerms">
                  <spring:message code="customerOrder.elasticFields" /></label>
                <form:input id="searchTerms" class="form-control" path="searchTerms" />
              </div>

              <sec:authorize access="hasAnyRole('${auth['CUSTOMER_ORDER_VIEW_DRS_STAFF']}')">
                <div class="form-group filterSearch" style="display: none;">
                  <label for="supplier">
                    <spring:message code="customerOrder.supplier" /></label>
                  <form:select id="supplier" class="form-control" path="supplierKcode" onchange="getBaseProducts()">
                    <form:option value="">All</form:option>
                    <c:forEach var="supplierKcodeToNameMap" items="${supplierKcodeToNameMap}">
                      <form:option value="${supplierKcodeToNameMap.key}">${supplierKcodeToNameMap.key}
                        ${supplierKcodeToNameMap.value}</form:option>
                    </c:forEach>
                  </form:select>
                </div>
              </sec:authorize>
              ...
              <div class="form-group filterSearch" style="display: none;">
                <label for="baseProduct">
                  <spring:message code="customerOrder.baseProduct" /></label>
                <form:select id="baseProduct" class="form-control" path="relatedBaseProductCode" onchange="getSKUs()">
                  <form:option value="">All</form:option>
                  <c:forEach var="productBaseCodeToNameMap" items="${productBaseCodeToNameMap}">
                    <form:option value="${productBaseCodeToNameMap.key}">${productBaseCodeToNameMap.key}
                      ${productBaseCodeToNameMap.value}</form:option>
                  </c:forEach>
                </form:select>
              </div>
              <div class="form-group filterSearch" style="display: none;">
                <label for="SKU">
                  <spring:message code="customerOrder.SKU" /></label>
            ...
            <div class="col-md-6">
              <div class="form-group">
                <label for="orderDateTimeZoneText">
                  <spring:message code="customerOrder.orderTime" /></label>
                <div class="form-row">
                  <div class="col">
                    <form:input type="text" id="orderDateStart" class="form-control datepicker" value=""
                      style="display: inline;cursor:default;background-color: white;" path="orderDateStart" />
                  </div>
                  <div class="col-1 text-center">---</div>
                  <div class="col">
                    <form:input type="text" id="orderDateEnd" class="form-control datepicker" value=""
                      style="display: inline;cursor:default;background-color: white;" path="orderDateEnd" />
                  </div>
                </div>
              </div>

              <div class="form-group filterSearch" style="display: none;">
                <label for="salesChannel">
                  <spring:message code="common.marketplace" /></label>
                <form:select id="salesChannel" class="form-control" path="salesChannelId"
                  onchange="updateOrderDateTimeZoneText()">
                  <form:option value="">All</form:option>
              ...
              <div class="form-group filterSearch" style="display: none;">
                <label for="orderStatus">
                  <spring:message code="customerOrder.orderStatus" /></label>
                <form:select id="orderStatus" class="form-control" path="orderStatus" value="${orderStatus}">
                  <form:option value="">All</form:option>
                    <sec:authorize
          ...
          <div class="row advancedSearch" id="adv_form">
            <div class="col-md-6">
              <sec:authorize access="hasAnyRole('${auth['CUSTOMER_ORDER_VIEW_DRS_STAFF']}')">
                <div class="form-group">
                  <label for="amazonMerchantSKU">
                    <spring:message code="customerOrder.amazonMerchantSKU" /></label>
                  <form:input id="amazonMerchantSKU" class="form-control" path="amazonMerchantSKU" />
                </div>
              </sec:authorize>
              <div class="form-group">
                <label for="asin">
                  <spring:message code="customerOrder.asin" /></label>
                <form:input id="asin" class="form-control" path="asin" />
              </div>
              <div class="form-group">
                <label for="transactionDateStart">
                  <spring:message code="customerOrder.transactionTimeUTC" /></label>
                <div class="form-row">
                  <div class="col">
                    <form:input id="transactionDateStart" class="form-control"
                      style="display: inline;cursor:default;background-color: white;" path="transactionDateStart" />
                  </div>
                  <div class="col-1 text-center">---</div>
                  <div class="col">
                    <form:input id="transactionDateEnd" class="form-control"
                      style="display: inline;cursor:default;background-color: white;" path="transactionDateEnd" />
                  </div>
                </div>
              </div>
              <div class="form-group">
                <label for="marketplaceOrderId">
                  <spring:message code="customerOrder.marketplaceOrderId" /></label>
            ...
            <div class="col-md-6">
              <div class="form-group">
                <label for="customerName">
                  <spring:message code="customerOrder.customerName" /></label>
                <form:input id="customerName" class="form-control" path="customerName" />
              </div>
              <div class="form-group">
                <sec:authorize access="hasAnyRole('${auth['CUSTOMER_ORDER_VIEW_DRS_STAFF']}')">
                  <label for="buyerEmail">
                    <spring:message code="customerOrder.buyerEmail" /></label>
                  <form:input id="buyerEmail" class="form-control" path="buyerEmail" />
                </sec:authorize>
              </div>
              <div class="form-group">
                <label for="customerName">
                  <spring:message code="customerOrder.promotionId" /></label>
                <form:input id="promotionId" class="form-control" path="promotionId" />
              </div>
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-md-12 text-right">
              ...
              <button class="btn btn-primary filterSearch" style="display: none;" type="submit"
                formaction="${normalSearch}">
              ...
              <sec:authorize access="hasAnyRole('${auth['CUSTOMER_ORDER_VIEW_SUPPLIER']}')">
                <button type="button" class="btn btn-default filterSearch" style="display: none;"
        ...
        <div id="customerOrderResultTitle" class="row py-2">
          <div class="col-md-12">
            <div class="page-heading">${searchResultTitle}</div>
            <c:if test="${!empty customerOrders}">
              <span>
                <spring:message code='customerOrder.pageInfo' arguments="${startCountPerPage};${endCountPerPage}"
                  argumentSeparator=";" />
                <c:choose>
                  <c:when test="${totalPages > 1}">
                    <spring:message code='customerOrder.pageInfoTotal' arguments="${totalRecords}" />
                  </c:when>
                </c:choose>
              </span>
            </c:if>
              ...
              <c:choose>
                <c:when test="${totalPages > 1}">
                  <c:url value="/CustomerOrderList" var="URL">
                    <c:forEach var="customerOrderCondition" items="${customerOrderConditions}">
                      <c:param name="${customerOrderCondition.key}" value="${customerOrderCondition.value}" />
                    </c:forEach>
                  </c:url>
                      ...
                      <ul class="pagination pagination-sm justify-content-center">
                        <c:choose>
                          <c:when test="${currentPageIndex != 1}">
                            <li class="page-item"><a class="page-link" href="${firstUrl}" data-pager="1">&lt;&lt;</a>
                            </li>
          ...
          <table id="customerOrder" class="table table-striped table-floated">
            <thead id="tableHeader">
              <tr style="height: 50px;">
                <th style="vertical-align: bottom; width: 135px;">
                  <spring:message code="customerOrder.orderTimeLocal" />
                </th>
                <th style="vertical-align: bottom; width: 135px;">
                  <spring:message code="customerOrder.orderTimeUTC" />
                </th>
                <th style="vertical-align: bottom; width: 135px;">
                  <spring:message code="customerOrder.transactionTimeUTC" />
                </th>
                <th style="vertical-align: bottom;">
                  <spring:message code="customerOrder.marketplaceOrderId" /> <br>
                  <spring:message code="customerOrder.buyer" />
                </th>
                <th style="vertical-align: bottom; width: 265px;">
                  <spring:message code="customerOrder.SKUCode" /> <br>
                  <spring:message code="customerOrder.productName" />
                </th>
                <th style="vertical-align: bottom;">
                  <spring:message code="customerOrder.orderStatus" />
                </th>
                <th style="vertical-align: bottom; width: 190px;">
                  <spring:message code="common.marketplace" /> <br>
                  <spring:message code="customerOrder.promotionId" />
                </th>
                <th class="text-left" style="vertical-align: bottom;">
                  <spring:message code="customerOrder.fulfillmentCenter" />
                </th>
                <th class="text-right" style="vertical-align: bottom;">
                  <spring:message code="customerOrder.qty" />
                </th>
                <th class="text-right" style="vertical-align: bottom;">
                  <spring:message code="customerOrder.actualRetailPrice" />
                </th>
              </tr>
            </thead>
            <tbody>
              <c:forEach items="${customerOrders}" var="customerOrder" varStatus="orderCount">
                <c:choose>
                  <c:when test="${customerOrder.orderStatus == 'Pending' || customerOrder.orderStatus == 'Canceled'}">
                    <tr class="text-muted">
                  </c:when>
                  <c:otherwise>
                    <tr>
                ...
                <td>
                  <c:choose>
                    <c:when test="${empty customerOrder.salesChannel || customerOrder.salesChannel == 'eBay'}">
                      ${customerOrder.marketplaceOrderId}
                    </c:when>
                    <c:otherwise>
                      <a id="ordrerCount${orderCount.index}" tabindex="${orderCount.index}" class="popTrigger"
                        data-toggle="popover" style="text-decoration: none; cursor: pointer;"
                  ...
                  <div class="pop" style="display: none;">
                      ...
                      <c:choose>
                        <c:when test="${customerOrder.orderStatus == 'Refunded'}">
                          <div class="col-md-6 text-left" style="color: #cc0000;">
                            ${customerOrder.orderStatus} <br> <span data-column="refundReason"></span>, <span
                    ...
                    <div id="fulfillmentService" class="row">
                      <div class="col-md-6 text-right">Fulfillment Service /
                        ID</div>
                      <div class="col-md-6 text-left" data-column="fulfillmentService"></div>
                    </div>
                ...
                <td style="white-space: nowrap; max-width: 200px;">${customerOrder.SKUCode}
                  <br>
                  <c:if test="${(fn:length(customerOrder.productName)) > 40}">
                    <c:set var="shortProductName" value="${fn:substring(customerOrder.productName,0,39)}" />
                ...
                <td style="white-space: nowrap; max-width: 175px;">
                  <sec:authorize access="hasAnyRole('${auth['CUSTOMER_ORDER_VIEW_DRS_STAFF']}')">
                    <c:choose>
                      <c:when test="${customerOrder.salesChannel == 'TrueToSource'}">
                        <a href="${marketplaceToOrderUrlPrefixMap[customerOrder.salesChannel]}${customerOrder.shopifyOrderId}"
                      ...
                      <c:otherwise>
                        <a href="${marketplaceToOrderUrlPrefixMap[customerOrder.salesChannel]}${customerOrder.marketplaceOrderId}"
                          target="_blank">${customerOrder.salesChannel}</a>
                      </c:otherwise>
                    </c:choose>
                  </sec:authorize>
                  ...
                  <c:if test="${(fn:length(customerOrder.promotionId)) > 30}">
                    <c:set var="shortPromotionId" value="${fn:substring(customerOrder.promotionId,0,29)}" />
                    <span title="${customerOrder.promotionId}" data-toggle="tooltip">${shortPromotionId }...</span>
                  </c:if>
                  <c:if test="${(fn:length(customerOrder.promotionId)) <= 30}">
