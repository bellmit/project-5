...
object AllOrderHandler {

  def props(drsCmdBus:ActorRef): Props =
    Props(new AllOrderHandler(drsCmdBus))
}

class AllOrderHandler(drsCmdBus:ActorRef) extends Actor with ActorLogging {
...
//        val gr =x.groupBy(x => (x.get("snapshot-date").get ,x.get("transaction-type").get ,x.get("fnsku").get,x.get("fulfillment-center-id").get))
        ...
        val gr =x.groupBy(x => (x.get("amazon-order-id").get ,x.get("purchase-date").get, x.get("sales-channel").get,x.get("sku").get))
