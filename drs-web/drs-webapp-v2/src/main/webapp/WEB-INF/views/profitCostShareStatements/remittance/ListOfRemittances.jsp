...
<div class="max-width">
	<div class="container-fluid">
		...
		<form:form  action="${action}" method="GET" name="RemittanceListSearch" modelAttribute="RemittanceListSearchCondition">
		<div class="row">
			<div class="col-md-12">
				<label class="control-label"><spring:message code='remittance.sender' /></label>
				<form:select id="sender" path="sndrKcode" class="form-control" style="width:20%;display:inline;">
				...
				<label class="control-label"><spring:message code='remittance.receiver' /></label>
				<form:select id="receiver" path="rcvrKcode" class="form-control" style="width:20%;display:inline;">
					<form:option value="">All</form:option>
					<c:forEach var="SenderAndReceiverCompany" items="${SenderAndReceiverCompanies}">
						<form:option value="${SenderAndReceiverCompany.key}" label="${SenderAndReceiverCompany.key} ${SenderAndReceiverCompany.value}" />
					</c:forEach>				
				</form:select>
				<input class="btn btn-default" type="submit" value="<spring:message code="remittance.search" />" >
			</div>
		</div>
		</form:form>
		<div class="row">
									...
									<c:forEach var="i" begin="${startPage}" end="${endPage}">
										...
										<c:choose>
											<c:when test="${i == currentPageIndex}">
												<li class="page-link active"><a class="page-link" href="${pageUrl}" data-pager="${i}"><c:out value="${i}" /></a></li>
											</c:when>
											<c:otherwise>
							...
							<nav>
								<ul class="pagination pagination-sm justify-content-center">
									...
									<c:forEach var="i" begin="${startPage}" end="${endPage}">
										...
										<c:choose>
											<c:when test="${i == currentPageIndex}">
												<li class="page-item active"><a class="page-link" href="${pageUrl}" data-pager="${i}"><c:out value="${i}" /></a></li>
											</c:when>
											<c:otherwise>
												<li class="page-item"><a class="page-link" href="${pageUrl}" data-pager="${i}"><c:out value="${i}" /></a></li>
											</c:otherwise>
